...
const Agents = ({agents,}:{agents: Array<any>}) => {
    ...
    return (
        <div className={styles.container} >
            <Button className={styles.add} onClick={() => {setAddShow(true), console.log('g')}}> 
            Add Agent
            </Button>
            ...
            {realAgents.map((agent, iter) => {
                    return (
                        <div key={iter}>
                            <AgentCard  key={iter} agent={agent} remove={removeAgent} i={iter} />
                        </div>
                    )
                })}
            </div>
            <Popup popupState={addShow} ref={addRef} crossHandler={() => {setAddShow(false)}}>
                <div className={styles.adddiv}>
                    <h1 className={styles.adddiv__heading}>Add an Agent</h1>
                    <Input value={adduser} type="text" className={styles.adddiv__input} placeholder='Username' onChange={setAdduser}></Input>
                    <Input value={password} type="password" className={styles.adddiv__input} placeholder='Password' onChange={setPassword}></Input>                
                    <SecondaryButton onClick={() => {console.log(adduser, password), addAgent(adduser, password)}} className={styles.adddiv__button}>Create</SecondaryButton>
                </div>
            </Popup>
        </div>
    )
